...
const navigate = useNavigate();
  ...
  return (
    <div className="pl-2 min-h-screen bg-gray-50">
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4 bg-[var(--base-color)] p-2">
        <h1 className="text-2xl font-bold uppercase text-[#035d67]">Purchase List</h1>
        ...
        <table className="min-w-full text-sm text-left table-auto">
          <thead className="bg-[var(--base-color)] text-gray-700 border border-gray-300">
            <tr className="border border-gray-300">
              ...
              <th className="px-4 py-2 border-r border-gray-300">Purchase No.</th>
              <th className="px-4 py-2 border-r border-gray-300">Invoice No.</th>
              <th className="px-4 py-2 border-r border-gray-300">Purchase Order No.</th>
              <th className="px-4 py-2 border-r border-gray-300">Vendor</th>
              <th className="px-4 py-2 border-r border-gray-300">Generated By</th>
              <th className="px-4 py-2 border-r border-gray-300">Date</th>
              <th className="px-4 py-2 border-r border-gray-300">Payment Terms</th>
              <th className="px-4 py-2">Action</th>
            </tr>
          </thead>
          <tbody>
            {currentPurchases.map((purchase, index) => (
              <tr key={purchase.id} className="border-t hover:bg-gray-50 transition duration-200 border border-gray-300">
                <td className="px-4 py-1 border-r border-gray-300">{(currentPage - 1) * itemsPerPage + index + 1}</td>
                <td className="px-4 py-1 border-r border-gray-300">{purchase.id}</td>
                <td className="px-4 py-1 border-r border-gray-300">{purchase.invoiceNo}</td>
                <td className="px-4 py-1 border-r border-gray-300">{purchase.poNo}</td>
                <td className="px-4 py-1 border-r border-gray-300">{purchase.vendor}</td>
                <td className="px-4 py-1 border-r border-gray-300">{purchase.generatedBy}</td>
                <td className="px-4 py-1 border-r border-gray-300">{purchase.date}</td>
                <td className="px-4 py-1 border-r border-gray-300">{purchase.paymentTerms}</td>
                <td className="px-4 py-1">
                  <Button
                    ...
                    hover="hover:bg-gray-200"
                    onClick={() => navigate(`/store/purchase-list/view/${purchase.id}`)}
                  />
                </td>
              </tr>
            ))}
