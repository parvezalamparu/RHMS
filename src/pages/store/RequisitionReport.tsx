...
const RequisitionReport = () => {
  ...
  const handleSort = (key: keyof RequisitionItem) => {
    setSortConfig((prev) => {
      if (prev?.key === key) {
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Requisiton Report");

    XLSX.writeFile(wb, "requisition_report.xlsx");
  };
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Requisiton Report", 14, 15);
        ...
        {viewMode === "table" ? (
          <table className="w-full text-sm text-gray-800 border-collapse">
            <thead className="bg-[var(--base-color)] text-xs font-semibold">
              <tr>
                {["SN", "Requisition ID", "Item Name", "Date", "Qty"].map(
                  (header, idx) => (
                    <th
                      key={idx}
                      ...
                      onClick={() =>
                        handleSort(
                          ["id", "requisitionId", "itemName", "date", "qty"][
                            idx
                          ] as keyof RequisitionItem
                        )
                      }
                    >
                      {header}{" "}
